...
class BOMItem(BaseModel):
    ...
    material_name: str = Field(
        ..., description="Indonesian material name for marketplace search"
    )
    english_name: str | None = Field(
        None, description="English material name for user display"
    )
    quantity: float = Field(..., gt=0, description="Quantity needed")
    unit: str = Field(
        ..., description="Unit of measurement", examples=["m2", "pcs", "kg", "liter"]
    )
...
class PriceLookupResponse(BaseModel):
    ...
    unit_price_idr: int = Field(..., ge=0, description="Price per unit in IDR")
    total_price_idr: int = Field(
        ..., ge=0, description="Total cost for requested quantity"
    )
    ...
    marketplace_url: str | None = Field(
        None, description="Link to product on marketplace"
    )
    affiliate_url: str | None = Field(
        None, description="Affiliate link for monetization"
    )
        ...
        json_schema_extra = {
            "example": {
                "materials": [
                    ...
                    {
                        "material_name": "tempered glass 8mm",
                        "quantity": 5,
                        "unit": "m2",
                    },
...
class BatchPriceLookupResponse(BaseModel):
    ...
    prices: list[PriceLookupResponse] = Field(
        ..., description="Individual price results"
    )
    ...
    cache_hits: int = Field(
        default=0, description="Number of prices from cache (no Apify cost)"
    )
