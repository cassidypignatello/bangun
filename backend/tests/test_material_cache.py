...
class TestBestSellerScoring:
    """Tests for Best Seller scoring algorithm with weighted ranking (Rating 40%, Sales 40%, Price 20%)"""

    def test_score_best_seller_price_weight(self):
        """Should weight price at 0.2 with lower = better"""
        from app.integrations.apify import score_best_seller

        # Lowest price product
        low_price = {"price_idr": 80000, "rating": 0, "sold_count": 0}
        # Highest price product
        high_price = {"price_idr": 120000, "rating": 0, "sold_count": 0}
        ...
        # Price contributes 0.2 weight
        assert low_score.total_score > high_score.total_score

    def test_score_best_seller_rating_weight(self):
        """Should weight rating at 0.4"""
        from app.integrations.apify import score_best_seller

        # High rating product
        high_rating = {"price_idr": 100000, "rating": 5.0, "sold_count": 0}
        # Low rating product
        low_rating = {"price_idr": 100000, "rating": 1.0, "sold_count": 0}
    ...
    def test_score_best_seller_sales_weight(self):
        """Should weight sales at 0.4 with logarithmic scale"""
        from app.integrations.apify import score_best_seller

        # High sales product
        high_sales = {"price_idr": 100000, "rating": 0, "sold_count": 10000}
        # Low sales product
        low_sales = {"price_idr": 100000, "rating": 0, "sold_count": 10}
        # No sales product
        no_sales = {"price_idr": 100000, "rating": 0, "sold_count": 0}
    ...
    def test_rank_best_sellers(self):
        ...
        products = [
            {"price_idr": 120000, "rating": 3.0, "sold_count": 50},
            {"price_idr": 80000, "rating": 4.5, "sold_count": 500},  # Should win (high rating + sales)
        ...
        assert len(ranked) == 2
        # Second product should win (best combination of rating/sales/price)
        assert ranked[0].product["price_idr"] == 80000
        # Scores should be descending
        assert ranked[0].total_score > ranked[1].total_score

    def test_rank_best_sellers_filters_insufficient_stock(self):
        """Should filter out products with insufficient stock for required quantity"""
        from app.integrations.apify import rank_best_sellers

        products = [
            {"price_idr": 80000, "rating": 5.0, "sold_count": 1000, "stock": 10},  # Not enough (need 50)
            {"price_idr": 90000, "rating": 4.5, "sold_count": 500, "stock": 100},  # Enough stock
            {"price_idr": 100000, "rating": 4.0, "sold_count": 200, "stock": 50},  # Exactly enough
            {"price_idr": 70000, "rating": 4.8, "sold_count": 800, "stock": 25},  # Not enough
        ]

        # Require 50 units
        ranked = rank_best_sellers(products, top_n=5, required_quantity=50)

        # Only 2 products have sufficient stock
        assert len(ranked) == 2
        # Product with stock=100 (price 90k) and stock=50 (price 100k) should remain
        assert all(r.product["stock"] >= 50 for r in ranked)
        # Best rated of the remaining should be first
        assert ranked[0].product["stock"] == 100  # Higher rated of the two

    def test_rank_best_sellers_allows_all_when_no_quantity_specified(self):
        """Should include all in-stock products when no required_quantity specified"""
        from app.integrations.apify import rank_best_sellers

        products = [
            {"price_idr": 80000, "rating": 5.0, "sold_count": 1000, "stock": 10},
            {"price_idr": 90000, "rating": 4.5, "sold_count": 500, "stock": 100},
            {"price_idr": 100000, "rating": 4.0, "sold_count": 200, "stock": 1},
        ]

        # No quantity requirement - all in-stock products should be included
